...
1. We run an anonymous function that has two parameters: sequelize and DataTypes.The
function will return the value of what is created by sequelize.define.
...
6. DataTypes.STRING is our value for the testdata property.Because we define it as a STRING value in the model, any information to be held in that column MUST be a string data - type.Remember that DataTypes is a parameter in the
function brought in through Sequelize.You can read more about Sequelize DataTypes here(Links to an external site.).Although JavaScript is a loosely typed language, Postgres wants to know what data types we 're adding to each of our columns. Sequelize is making us declare the data types that we'
ll be storing.
